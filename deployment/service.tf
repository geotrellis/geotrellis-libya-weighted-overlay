...
resource "aws_ecs_cluster" "libya" {

  lifecycle {
    create_before_destroy = true
  }

  name = "LIBYA_${var.stack_name}"
}

# Template for container definition, allows us to inject environment
data "template_file" "ecs_libya_task" {
...
resource "aws_ecs_task_definition" "libya" {
  family                = "libya_${var.stack_name}"
  container_definitions = "${data.template_file.ecs_libya_task.rendered}"
}

# Defines running an ECS task as a service
resource "aws_ecs_service" "libya" {
  name                               = "LIBYA_${var.stack_name}"
  cluster                            = "${aws_ecs_cluster.libya.id}"
  task_definition                    = "${aws_ecs_task_definition.libya.family}:${aws_ecs_task_definition.libya.revision}"
  ...
  load_balancer {
    elb_name       = "${aws_elb.libya.name}"
    container_name = "libya-tile-server"
    container_port = 8777
  }
}

# Load balance among all running containers
resource "aws_elb" "libya" {
...
resource "aws_launch_configuration" "ecs" {
  name = "ECS ${aws_ecs_cluster.libya.name}"
  ...
  user_data = "#!/bin/bash\necho ECS_CLUSTER='${aws_ecs_cluster.libya.name}' > /etc/ecs/ecs.config"
}
...
resource "aws_autoscaling_group" "ecs" {
  ...
  tag {
    key                 = "Name"
    value               = "ECS ${aws_ecs_cluster.libya.name}"
